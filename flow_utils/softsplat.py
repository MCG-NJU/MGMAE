...
def cupy_kernel(strFunction, objVariables):
    ...
    while True:
        ...
        strKernel = strKernel.replace(objMatch.group(0),
                                      '(' + str.join('+', strIndex) + ')')
    # end
...
class _FunctionSoftsplat(torch.autograd.Function):
    ...
    def forward(self, input, flow):
        ...
        intInputDepth, intInputHeight, intInputWidth = input.shape[
            1], input.shape[2], input.shape[3]
        intFlowDepth, intFlowHeight, intFlowWidth = flow.shape[1], flow.shape[
            2], flow.shape[3]
    ...
    def backward(self, gradOutput):
        ...
        gradInput = input.new_zeros([
            intSamples, intInputDepth, intInputHeight, intInputWidth
        ]) if self.needs_input_grad[0] == True else None
        gradFlow = input.new_zeros([
            intSamples, intFlowDepth, intFlowHeight, intFlowWidth
...
def FunctionSoftsplat(tenInput, tenFlow, tenMetric, strType):
    ...
    if strType == 'average':
        tenInput = torch.cat([
            tenInput,
            tenInput.new_ones(tenInput.shape[0], 1, tenInput.shape[2],
                              tenInput.shape[3])
        ], 1)
...
class Softsplat(nn.Module):
    ...
    def __init__(self, strType='softmax', z=None):
...
def BackWarp(tenInput, tenFlow, device):
    k = (str(get_rank()), str(tenFlow.size()))
    if k not in backwarp_tenGrid:
        tenHorizontal = torch.linspace(-1.0,
                                       1.0,
                                       tenFlow.shape[3],
                                       device=device).view(
                                           1, 1, 1, tenFlow.shape[3]).expand(
                                               tenFlow.shape[0], -1,
                                               tenFlow.shape[2], -1)
        tenVertical = torch.linspace(-1.0,
                                     1.0,
                                     tenFlow.shape[2],
                                     device=device).view(
                                         1, 1, tenFlow.shape[2],
    ...
    tenFlow = torch.cat([
        tenFlow[:, 0:1, :, :] / ((tenInput.shape[3]) / 2.0),
        tenFlow[:, 1:2, :, :] / ((tenInput.shape[2]) / 2.0)
    ], 1)
